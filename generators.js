...
const GENERATORS = {
	pixarLamps: {
		description: 'SMASH THE "I"!',
		sliders: ["size", "aspectRatio", "angle", "hue", "brightness", "jump"],
		landmarks: {
			squish: [0.5, 0.04, 0.58, 0.0, 0.5, 0.4],
			jump: [0.26, 0.96, 0.49, 0.28, 0.51, 0.87, 0.54],
			disappearing_I: [0.0, 0.74, 0.46, 0.8, 0.0, 0.09, 0.27],
			kick: [0.66, 0.88, 0.91, 0.82, 0.65, 0.52],
			stand: [0.29, 0.42, 0.45, 0.45, 0.31, 0.48],
		},
		...
		draw(p, t, dna) {
			...
			drawEye(p, {
				...
				y: 0,
				eyeSize: size / 10,
				innerColor: [hue, 100, brightness + 20],
			});
			p.pop();
			...
			p.fill(60);
			p.ellipse(20, 0, 20, 10);

			p.noFill();
			p.strokeWeight(4);
			p.stroke(51);
			p.beginShape();
			p.vertex(4 * bulbSize, lampHeight + bulbSize);
			p.vertex(6 * bulbSize, lampHeight + 2 * bulbSize);
			p.vertex(4 * bulbSize + 10, lampHeight / 2);
			p.vertex(2 * bulbSize + 10, lampHeight + 2 * bulbSize);
			p.endShape(p.CLOSE);
			...
			p.beginShape();
			p.vertex(4 * bulbSize + 10, lampHeight / 2);
			p.vertex(4 * bulbSize, -10);
			p.vertex(4 * bulbSize, -50);
			p.endShape(p.CLOSE);

			p.fill(9, 15, 75);
			p.noStroke();
			p.beginShape();
			p.vertex(0, lampHeight + 1 + 2 * bulbSize);
			p.vertex(4 * bulbSize, lampHeight + bulbSize);
			p.vertex(4 * bulbSize, lampHeight - bulbSize);
			p.vertex(0, lampHeight - 2 * bulbSize);
			p.endShape();

			p.fill(9, 15, 91);
			p.circle(0, lampHeight, bulbSize * 2);
			p.fill(hue, 100, brightness);
			p.circle(-5, lampHeight, bulbSize);
	...
	alphabet: {
		...
		sliders: ["size", "aspectRatio", "angle", "hue", "brightness", "letter"],
		landmarks: {
			big_random_on_reload: [1, 1, 0.5, 0.5, 0.64, random],
			small_random_on_reload: [0.2, random, 0.04, 0.64, 0.35, random],
		...
		draw(p, t, dna) {
			...
			// Get letter
			let letterIndex = Math.floor(dna[5] * 27);
			let letter = letters.charAt(letterIndex);

			let color = dna[3] * 360;

			let brightness = dna[4] * 50 + 50;

			let angle = -Math.PI / 4 + (Math.PI / 2) * dna[2];

			let aspectRatio = dna[1] * 1.2 + 0.8;
			...
			let eyeHeight = h * 0.8;
			let eyeSize = 5;
			let eyeColor = [color, 100, brightness + 20];
